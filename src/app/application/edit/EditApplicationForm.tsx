...
export default function EditApplicationForm({ application, token }: { application: Record<string, any>; token: string }) {
  ...
  const [occupants, setOccupants] = useState<Array<{ name: string; email: string; relationship: string }>>(
    application.additional_occupants || []
  );
  ...
  const [prevResidences, setPrevResidences] = useState<Array<Record<string, string>>>(
    application.previous_residences || []
  );
  ...
  const [references, setReferences] = useState<Array<Record<string, string>>>(
    application.references || application.applicant_references || [{ name: "", phone: "", email: "", relationship: "", length: "" }]
  );

  // Section 9: Pets
  const [hasPets, setHasPets] = useState<boolean>(application.has_pets || false);
  const [pets, setPets] = useState<Array<Record<string, string>>>(application.pets || []);
  ...
  const [consent, setConsent] = useState<boolean>(application.consent_agreed || false);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/api/applications/edit?token=${encodeURIComponent(token)}`, {
        method: "PUT",
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 max-w-md mx-auto mb-4">
          Your application has been updated. Your edit link has been extended for another 7 days in case you need to make further changes.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2">
      ...
      <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-lg mb-6 text-sm">
        <strong>Editing your application.</strong> All changes will update your application on file. You can only edit while your application is under review.
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
          ...
          <input type="text" name="full_name" required defaultValue={application.full_name} />
        </div>
        <div>
          <label>Date of Birth<RequiredStar /></label>
          <input type="date" name="date_of_birth" required defaultValue={application.date_of_birth} />
        </div>
        <div>
          <label>Email Address<RequiredStar /></label>
          <input type="email" name="email" required defaultValue={application.email} />
        </div>
        <div>
          <label>Phone Number<RequiredStar /></label>
          <input type="tel" name="phone" required defaultValue={application.phone}
            onChange={(e) => { e.target.value = formatPhone(e.target.value); }} />
        </div>
        <div>
          <label>Desired Move-in Date<RequiredStar /></label>
          <input type="date" name="desired_move_in" required defaultValue={application.desired_move_in} />
        </div>
        <div>
          <label>Applying As<RequiredStar /></label>
          <select name="applying_as" required defaultValue={application.applying_as}>
      ...
      <p className="text-gray-500 text-sm mb-4">List anyone else who will be living in the property.</p>
      {occupants.map((occ, i) => (
        <div key={i} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-lg relative">
          <div>
            ...
            <input type="text" value={occ.name} onChange={(e) => { const u = [...occupants]; u[i].name = e.target.value; setOccupants(u); }} />
          </div>
          <div>
            <label>Email</label>
            <input type="email" value={occ.email} onChange={(e) => { const u = [...occupants]; u[i].email = e.target.value; setOccupants(u); }} />
          </div>
          <div>
            <label>Relationship</label>
            <input type="text" value={occ.relationship} onChange={(e) => { const u = [...occupants]; u[i].relationship = e.target.value; setOccupants(u); }} />
          </div>
          <button type="button" className="absolute top-2 right-2 text-red-400 hover:text-red-600" onClick={() => setOccupants(occupants.filter((_, j) => j !== i))}>✕</button>
        </div>
      ))}
      <button type="button" className="text-sm font-medium px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50"
      ...
      <p className="text-gray-500 text-sm mb-4">Only fill this out if applicable.</p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label>Guarantor Name</label>
          <input type="text" name="guarantor_name" defaultValue={application.guarantor_name || ""} />
        </div>
        <div>
          <label>Guarantor Email</label>
          <input type="email" name="guarantor_email" defaultValue={application.guarantor_email || ""} />
        </div>
        <div>
          <label>Guarantor Phone</label>
          <input type="tel" name="guarantor_phone" defaultValue={application.guarantor_phone || ""}
            onChange={(e) => { e.target.value = formatPhone(e.target.value); }} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select name="current_housing_type" required defaultValue={application.current_housing_type}>
        ...
        <div>
          ...
          <input type="text" placeholder="MM/YYYY" name="current_date_from" required defaultValue={application.current_date_from}
            onChange={(e) => { e.target.value = formatDate(e.target.value); }} />
        </div>
        <div>
          <label>Date To (Month/Year or Present)<RequiredStar /></label>
          <input type="text" name="current_date_to" placeholder="MM/YYYY or Present" required defaultValue={application.current_date_to} />
        </div>
        <div>
          <label>Monthly Rent<RequiredStar /></label>
          <input type="number" name="current_monthly_rent" step="0.01" required defaultValue={application.current_monthly_rent} />
        </div>
        <div>
          <label>Landlord/Manager Name<RequiredStar /></label>
          <input type="text" name="current_landlord_name" required defaultValue={application.current_landlord_name} />
        </div>
        <div>
          <label>Landlord Email</label>
          <input type="email" name="current_landlord_email" defaultValue={application.current_landlord_email || ""} />
        </div>
        <div>
          <label>Landlord Phone<RequiredStar /></label>
          <input type="tel" name="current_landlord_phone" required defaultValue={application.current_landlord_phone}
            onChange={(e) => { e.target.value = formatPhone(e.target.value); }} />
        </div>
      </div>
      ...
      {prevResidences.map((res, i) => (
        <div key={i} className="p-4 bg-gray-50 rounded-lg mb-4 relative">
          <button type="button" className="absolute top-2 right-2 text-red-400 hover:text-red-600"
            onClick={() => setPrevResidences(prevResidences.filter((_, j) => j !== i))}>✕</button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={res.housing_type || ""} onChange={(e) => { const u = [...prevResidences]; u[i] = { ...u[i], housing_type: e.target.value }; setPrevResidences(u); }}>
                <option value="">Select...</option><option value="Rental">Rental</option><option value="Owned">Owned</option><option value="Other">Other</option>
              </select>
            </div>
            <div className="md:col-span-2">
            ...
            <div><label>City</label><input type="text" value={res.city || ""} onChange={(e) => { const u = [...prevResidences]; u[i] = { ...u[i], city: e.target.value }; setPrevResidences(u); }} /></div>
            <div>
              <label>State</label>
      ...
      <button type="button" className="text-sm font-medium px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50"
        onClick={() => setPrevResidences([...prevResidences, {}])}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select name="work_status" required defaultValue={application.work_status}>
            <option value="">Select...</option>
        ...
        <div><label>Employer Name<RequiredStar /></label><input type="text" name="employer_name" required defaultValue={application.employer_name} /></div>
        <div><label>Job Title<RequiredStar /></label><input type="text" name="job_title" required defaultValue={application.job_title} /></div>
        <div><label>Gross Monthly Income<RequiredStar /></label><input type="number" name="gross_monthly_income" step="0.01" required defaultValue={application.gross_monthly_income} /></div>
        <div>
          <label>Date From<RequiredStar /></label>
          <input type="text" placeholder="MM/YYYY" name="employment_date_from" required defaultValue={application.employment_date_from}
            onChange={(e) => { e.target.value = formatDate(e.target.value); }} />
        </div>
        <div><label>Date To<RequiredStar /></label><input type="text" name="employment_date_to" placeholder="MM/YYYY or Present" required defaultValue={application.employment_date_to} /></div>
        <div><label>Supervisor Name</label><input type="text" name="supervisor_name" defaultValue={application.supervisor_name || ""} /></div>
        <div><label>Supervisor Phone</label><input type="tel" name="supervisor_phone" defaultValue={application.supervisor_phone || ""} onChange={(e) => { e.target.value = formatPhone(e.target.value); }} /></div>
        <div><label>Supervisor Email</label><input type="email" name="supervisor_email" defaultValue={application.supervisor_email || ""} /></div>
      </div>
      ...
      {references.map((ref, i) => (
        <div key={i} className="p-4 bg-gray-50 rounded-lg mb-4 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label>Length of Acquaintance</label><input type="text" value={ref.length || ref.length_of_acquaintance || ""} onChange={(e) => { const u = [...references]; u[i] = { ...u[i], length: e.target.value }; setReferences(u); }} /></div>
          </div>
        </div>
      ))}
      <button type="button" className="text-sm font-medium px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50"
        onClick={() => setReferences([...references, { name: "", phone: "", email: "", relationship: "", length: "" }])}>
      ...
      <div className="mb-4">
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" checked={hasPets} onChange={(e) => { setHasPets(e.target.checked); if (!e.target.checked) setPets([]); }} className="w-5 h-5" />
          ...
          <div key={q.key} className="p-4 bg-gray-50 rounded-lg">
            ...
            <div className="flex gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name={`screening_${q.key}`} checked={screening[q.key]?.answer === true}
                  onChange={() => setScreening({ ...screening, [q.key]: { ...screening[q.key], answer: true } })} className="w-4 h-4" />
                Yes
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name={`screening_${q.key}`} checked={screening[q.key]?.answer === false}
                  onChange={() => setScreening({ ...screening, [q.key]: { ...screening[q.key], answer: false, details: "" } })} className="w-4 h-4" />
                No
              </label>
            </div>
            {screening[q.key]?.answer && (
              <div className="mt-3">
                ...
                <textarea rows={2} value={screening[q.key]?.details || ""}
                  onChange={(e) => setScreening({ ...screening, [q.key]: { ...screening[q.key], details: e.target.value } })} className="mt-1" />
              </div>
            )}
          </div>
        ))}
      ...
      <div className="bg-gray-50 p-6 rounded-lg mb-6">
        <p className="text-sm leading-relaxed text-gray-700 italic">
          I hereby authorize a review and full disclosure of all records concerning myself. I authorize current and past landlords, employers, financial institutions, personal references, and courts of law to release information regarding my rental, employment, credit, and/or criminal history. I agree to indemnify and hold harmless Geaux Home Buyers LLC and its agents from claims arising from complying with this request. A copy of this authorization may be accepted as an original.
        </p>
      </div>
      <div className="space-y-4">
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} required className="w-5 h-5 mt-0.5" />
          <span>I agree to the above authorization<RequiredStar /></span>
        </label>
        <div>
          <label>Typed Full Legal Name (E-Signature)<RequiredStar /></label>
          <input type="text" name="signature_name" required placeholder="Type your full legal name"
            className="font-serif text-lg" defaultValue={application.signature_name} />
        </div>
